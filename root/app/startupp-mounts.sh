...
echo -e "defaults,nonempty,cache.symlinks=true,cache.readdir=true,cache.files=auto-full,direct_io=true,cache.open=259200,cache.statfs=259200,cache.attr=259200,cache.entry=259200,category.create=epff,minfreespace=0,allow_other,dropcacheonclose=true,security_capability=false,xattr=nosys,statfs_ignore=ro,use_ino,hard_remove,nullrw=true,async_read=false,umask=0002,noatime,security_capability=false,xattr=nosys,statfs_ignore=nc,statfs=full,threads=100,cache.writeback=true,posix_acl=false,symlinkify=true" >> /tmp/mergerfs_mount_file
