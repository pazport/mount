...
echo -e "defaults,nonempty,cache.symlinks=true,cache.files=auto-full,cache.open=259200,cache.statfs=259200,cache.attr=259200,cache.entry=259200,cache.negative_entry=259200,category.create=epff,minfreespace=0,allow_other,dropcacheonclose=true,security_capability=false,xattr=nosys,statfs_ignore=ro,use_ino,hard_remove,async_read=true,umask=0002,noatime,security_capability=false,xattr=nosys,statfs_ignore=nc,statfs=full,threads=100,cache.writeback=true,posix_acl=false,symlinkify=true,fsname=mergerfs" >> /tmp/mergerfs_mount_file
